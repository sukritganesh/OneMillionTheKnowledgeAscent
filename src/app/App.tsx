...
function DialogLayer(props: {
  ...
  if (dialog.kind === 'create-profile') return <Modal title="Create a profile" onClose={props.onClose} actions={<><button type="button" className="secondary-button" onClick={props.onClose}>Cancel</button><button type="button" className="primary-button" onClick={props.onCreate} disabled={!props.createName.trim()}>Create profile</button></>}><label className="field-stack"><span>Display name</span><input autoComplete="off" maxLength={40} type="text" value={props.createName} onChange={(event) => props.onName(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && props.createName.trim()) props.onCreate(); }} /><small className="field-help">Use a name up to 40 characters long. You can share a name with another player.</small></label>{props.error && <p className="field-error" role="alert">{props.error}</p>}</Modal>;
  if (dialog.kind === 'delete-profile') return <Modal destructive title={`Delete ${dialog.profile.displayName}?`} onClose={props.onClose} actions={<><button type="button" className="secondary-button" onClick={props.onClose}>Keep Profile</button><button type="button" className="danger-button" onClick={() => props.onDelete(dialog.profile)}>Delete Permanently</button></>}><p>This permanently deletes this player's stats, question history, set progress, and past games.</p><div className="notice notice--error"><span>!</span><div>This player's saved game will also be deleted. You can only recover it from a backup.</div></div>{props.error && <p className="field-error">{props.error}</p>}</Modal>;
  if (dialog.kind === 'take-control') return <Modal title="Game open in another tab" onClose={props.onClose} actions={<><button type="button" className="secondary-button" onClick={props.onClose}>Leave It There</button><button type="button" className="primary-button" onClick={props.onTakeControl}>Take Control Here</button></>}><p>Continue playing here? The game will pause in the other tab.</p>{props.error && <p className="field-error">{props.error}</p>}</Modal>;
  if (dialog.kind === 'reset-all') return <Modal destructive title="Reset all local data?" onClose={props.onClose} actions={<><button type="button" className="secondary-button" onClick={props.onClose}>Cancel</button><button type="button" className="danger-button" onClick={props.onReset}>Reset Everything</button></>}><p>This deletes all players, past games, settings, the saved game, and added question packs in this browser.</p><div className="notice notice--error"><span>!</span><div>Download a backup first if you want to keep a copy. This cannot be undone.</div></div></Modal>;
  if (dialog.kind === 'restore') return <Modal destructive title="Replace local data with this backup?" onClose={props.onClose} actions={<><button type="button" className="secondary-button" onClick={props.onClose}>Cancel</button><button type="button" className="danger-button" onClick={() => props.onRestore(dialog.source)}>Restore backup</button></>}><div className="modal-card-grid"><div className="data-stat"><span>Profiles</span><strong>{dialog.summary.profiles}</strong></div><div className="data-stat"><span>Completed runs</span><strong>{dialog.summary.runs}</strong></div><div className="data-stat"><span>Custom packs</span><strong>{dialog.summary.importedPacks}</strong></div><div className="data-stat"><span>Active save</span><strong>{dialog.summary.hasActiveSave ? 'Yes' : 'No'}</strong></div></div><p>This backup has been checked. Restoring it will replace the data in this browser. If the restore fails, your current data will be kept.</p>{props.error && <p className="field-error">{props.error}</p>}</Modal>;
  return <Modal title="Import this profile?" onClose={props.onClose} actions={<><button type="button" className="secondary-button" onClick={props.onClose}>Cancel</button><button type="button" className="primary-button" onClick={() => props.onProfileImport(dialog.source)}>Create New Profile</button></>}><p>{dialog.summary}</p><p>This adds a separate player. Your existing players and their history will stay as they are.</p>{props.error && <p className="field-error">{props.error}</p>}</Modal>;
}
